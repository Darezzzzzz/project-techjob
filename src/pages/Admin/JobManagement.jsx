    ...
    <div className="job-management-container">
      ...
      <div className="table-container">
        <div className="table-header-controls">
          ...
          <button className="collapse-btn" onClick={() => setTableCollapsed(p => !p)}>
            {tableCollapsed ? '▼' : '▲'}
          </button>
        </div>

        {!tableCollapsed && (filteredJobs.length > 0 ? (
      ...
      {/* กล่องงานรอดำเนินการ */}
      <div className="inprogress-box pending-box">
        <div className="inprogress-box-header">
          <Clock size={18} className="pending-icon" />
          <h3>งานที่รอดำเนินการ ({filteredJobs.filter(j => j.status === 'รอดำเนินการ').length})</h3>
        </div>
        {filteredJobs.filter(j => j.status === 'รอดำเนินการ').length > 0 ? (
          <div className="inprogress-list">
            {filteredJobs.filter(j => j.status === 'รอดำเนินการ').map(job => (
              <div key={job.id} className="inprogress-card pending-card">
                <div className="inprogress-card-left">
                  <span className="inprogress-id pending-id">{job.id}</span>
                  <span className="inprogress-name">{job.name}</span>
                </div>
                <div className="inprogress-card-right">
                  <span className="inprogress-dept">{job.department && job.department !== 'ยังไม่มอบหมายแผนก' ? job.department : 'ยังไม่มอบหมาย'}</span>
                  <span className="inprogress-location">📍 {job.location || 'ไม่ระบุ'}</span>
                  <div className="job-actions-cell">
                    {(!job.department || job.department === 'ยังไม่มอบหมายแผนก') && (
                      <button className="assign-btn" onClick={() => openAssignModal(job)}><ClipboardList size={14} />มอบหมาย</button>
                    )}
                    <button className="edit-btn" onClick={() => handleEditJob(job)}><Edit size={14} />แก้ไข</button>
                    <button className="delete-btn" onClick={() => handleDeleteJob(job.id)}><Trash2 size={14} />ลบ</button>
                  </div>
                </div>
              </div>
            ))}
          ...
          <p className="inprogress-empty">ไม่มีงานที่รอดำเนินการ</p>
        )}
      </div>

      {/* กล่องงานกำลังดำเนินการ */}
      <div className="inprogress-box">
        <div className="inprogress-box-header">
          <CircleDot size={18} className="inprogress-icon" />
          <h3>งานที่กำลังดำเนินการ ({filteredJobs.filter(j => j.status === 'กำลังดำเนินการ').length})</h3>
        </div>
        {filteredJobs.filter(j => j.status === 'กำลังดำเนินการ').length > 0 ? (
          <div className="inprogress-list">
            {filteredJobs.filter(j => j.status === 'กำลังดำเนินการ').map(job => (
              <div key={job.id} className="inprogress-card">
                <div className="inprogress-card-left">
                  <span className="inprogress-id">{job.id}</span>
                  <span className="inprogress-name">{job.name}</span>
                </div>
                <div className="inprogress-card-right">
                  <span className="inprogress-dept">{job.department || 'ไม่ระบุแผนก'}</span>
                  <span className="inprogress-location">📍 {job.location || 'ไม่ระบุ'}</span>
                  <span className="inprogress-tech">👤 {job.technician && job.technician !== 'ยังไม่มอบหมายแผนก' ? job.technician : 'ยังไม่มีช่าง'}</span>
                  <div className="job-actions-cell">
                    <button className="edit-btn" onClick={() => handleEditJob(job)}><Edit size={14} />แก้ไข</button>
                    <button className="delete-btn" onClick={() => handleDeleteJob(job.id)}><Trash2 size={14} />ลบ</button>
                  </div>
          ...
          <p className="inprogress-empty">ไม่มีงานที่กำลังดำเนินการอยู่</p>
        )}
      </div>

      {/* กล่องงานเสร็จสิ้น */}
      <div className="inprogress-box completed-box">
        <div className="inprogress-box-header">
          <CheckCircle size={18} className="completed-icon" />
          <h3>งานที่เสร็จสิ้น ({filteredJobs.filter(j => j.status === 'เสร็จสิ้น' || j.status === 'ผ่านการตรวจสอบ').length})</h3>
        </div>
        {filteredJobs.filter(j => j.status === 'เสร็จสิ้น' || j.status === 'ผ่านการตรวจสอบ').length > 0 ? (
          <div className="inprogress-list">
            {filteredJobs.filter(j => j.status === 'เสร็จสิ้น' || j.status === 'ผ่านการตรวจสอบ').map(job => (
              <div key={job.id} className="inprogress-card completed-card">
                <div className="inprogress-card-left">
                  <span className="inprogress-id completed-id">{job.id}</span>
                  <span className="inprogress-name">{job.name}</span>
                </div>
                <div className="inprogress-card-right">
                  <span className="inprogress-dept">{job.department || 'ไม่ระบุแผนก'}</span>
                  <span className="inprogress-location">📍 {job.location || 'ไม่ระบุ'}</span>
                  <span className="inprogress-tech">👤 {job.technician && job.technician !== 'ยังไม่มอบหมายแผนก' ? job.technician : 'ไม่ระบุ'}</span>
                  <div className="job-actions-cell">
                    <button className="edit-btn" onClick={() => handleEditJob(job)}><Edit size={14} />แก้ไข</button>
                    <button className="delete-btn" onClick={() => handleDeleteJob(job.id)}><Trash2 size={14} />ลบ</button>
                  </div>
          ...
          <p className="inprogress-empty">ยังไม่มีงานที่เสร็จสิ้น</p>
        )}
      </div>
